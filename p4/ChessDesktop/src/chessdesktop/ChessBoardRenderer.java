...
public class ChessBoardRenderer {
	...
	void drawPiece(Canvas canvas, Chess.ChessPiece piece, double minX, double minY, double width, double height) {
		...
		if (piece == movingPiece){
                    drawAvailablePositions(canvas, movingPiece); // AQUI?????????????????????????????????????????????????????????????
		...
		switch(piece.getType()) {
			...
			case BISHOP:
				...
				break;
			case KNIGHT:
                                drawTriangle(gc, minX, minY + height * 0.1, width, height * 0.45);
				gc.fillRect(minX, minY + height * 0.5, width, height * 0.5);
				break;
			case PAWN: 
	...
	void drawPiece(Canvas canvas, Bounds boardBounds, Chess.ChessPiece piece) {
        ...
        void drawSquareAt(Canvas canvas, int aColumn, int aRow, Color aColor) {
            Bounds bounds = canvas.getBoundsInLocal();
            GraphicsContext gc = canvas.getGraphicsContext2D();
            gc.setFill(aColor);
            ...
            double width = boardBounds.getWidth() / 8;
            double height = boardBounds.getHeight() / 8;
            double xmin = boardBounds.getMinX() + aColumn * width;
            double xmax = boardBounds.getMinX() + (aColumn + 1) * width;
            double ymin = boardBounds.getMinY() + aRow * height;
            double ymax = boardBounds.getMinY() + (aRow + 1) * height;
			
            gc.fillRect(xmin, ymin, width + 1, height + 1);	
	}
        
        void drawAvailablePositions(Canvas canvas, ChessPiece aPiece) {
            PiecePosition[] aPositions;
            aPositions = aPiece.getAvailablePositions(board);
            for (PiecePosition p : aPositions) {
                int c = p.getColumn();
                int r = p.getRow();
                drawSquareAt(canvas, c, r, Color.GREY);
            }
        }	
	...
	void draw(Canvas canvas) {
		...
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
                            if ((i + j) % 2 == 0)
                                drawSquareAt(canvas, i, j, Color.AQUA);
                            else
                                drawSquareAt(canvas, i, j, Color.YELLOW);
			}
                }
